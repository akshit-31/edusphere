...
class _TeacherProfileEditScreenState extends State<TeacherProfileEditScreen>
  ...
  final _addressCtrl   = TextEditingController(text: '123 Education Street, Manhattan, New York, USA');
  ...
  final _facebookCtrl  = TextEditingController(text: 'https://facebook.com/emma.johnson');
  final _twitterCtrl   = TextEditingController(text: 'https://twitter.com/emma_edu');
  final _instaCtrl     = TextEditingController(text: 'https://instagram.com/emma_edu');

  // ── Documents ────────────────────────────────────────────────────
  final List<Map<String, String>> _docs = [
    {'name': 'Resume / CV',            'type': 'PDF', 'size': '2.4 MB', 'date': '12 May 2024'},
    {'name': 'Degree Certificate (M.Sc)', 'type': 'PDF', 'size': '1.8 MB', 'date': '12 May 2024'},
    {'name': 'B.Ed Certificate',       'type': 'PDF', 'size': '1.6 MB', 'date': '12 May 2024'},
    {'name': 'Experience Certificate', 'type': 'PDF', 'size': '1.2 MB', 'date': '12 May 2024'},
    {'name': 'ID Proof (Aadhar Card)', 'type': 'JPG', 'size': '1.5 MB', 'date': '12 May 2024'},
    {'name': 'Profile Photo',          'type': 'JPG', 'size': '0.5 MB', 'date': '12 May 2024'},
  ];
  ...
  Future<void> _loadData() async {
    ...
    setState(() {
      ...
      _mobileCtrl.text = prefs.getString('teacher_mobile') ?? '+1 (555) 123-4557';
      _emailCtrl.text = prefs.getString('teacher_email') ?? 'emma.johnson@edusphere.com';
      _addressCtrl.text = prefs.getString('teacher_address') ?? '123 Education Street, Manhattan, New York, USA';
      _emergNameCtrl.text = prefs.getString('teacher_emerg_name') ?? 'Michael Johnson';
      _emergRelCtrl.text = prefs.getString('teacher_emerg_rel') ?? 'Brother';
      _emergPhCtrl.text = prefs.getString('teacher_emerg_ph') ?? '+1 (555) 967-9143';
      
      _empIdCtrl.text = prefs.getString('teacher_emp_id') ?? 'TCH1024';
      _deptCtrl.text = prefs.getString('teacher_dept') ?? 'Mathematics';
      _designCtrl.text = prefs.getString('teacher_design') ?? 'Senior Mathematics Teacher';
      ...
      _bioCtrl.text = prefs.getString('teacher_bio') ?? 'Passionate mathematics teacher with 6+ years of experience in teaching secondary and senior secondary students.';
      _linkedinCtrl.text = prefs.getString('teacher_linkedin') ?? 'https://linkedin.com/in/emma-johnson';
      _websiteCtrl.text = prefs.getString('teacher_website') ?? 'https://www.emmaeducation.com';
      
      _subjects = prefs.getStringList('teacher_subjects') ?? ['Mathematics', 'Algebra', 'Calculus'];
      _prefMode = prefs.getString('teacher_pref_mode') ?? 'Both';
      _availability.clear();
      _availability.addAll(prefs.getStringList('teacher_availability') ?? ['Mon', 'Tue', 'Wed', 'Thu', 'Fri']);
  ...
  void _addSubject() {
    ...
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      title: Text('Add Subject', style: GoogleFonts.inter(fontWeight: FontWeight.w800)),
      content: TextField(
        controller: ctrl,
        decoration: InputDecoration(hintText: 'Subject name', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12.r))),
      ),
      actions: [
        ...
        ElevatedButton(
          ...
          onPressed: () {
            if (ctrl.text.trim().isNotEmpty) setState(() => _subjects.add(ctrl.text.trim()));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Edit Profile', style: GoogleFonts.inter(fontSize: 18.sp, fontWeight: FontWeight.w800, color: AppColors.textDark)),
        centerTitle: true,
        actions: [
          Padding(
            ...
            child: TextButton(
              ...
              child: Text('Save', style: GoogleFonts.inter(fontSize: 15.sp, fontWeight: FontWeight.w800, color: widget.theme.primary)),
            ),
          ),
        ],
        bottom: TabBar(
          ...
          indicatorWeight: 2.5,
          labelStyle: GoogleFonts.inter(fontSize: 11.sp, fontWeight: FontWeight.w700),
          tabs: [
            Tab(icon: Icon(Icons.person_outline_rounded, size: 20.sp), text: 'Personal'),
            Tab(icon: Icon(Icons.work_outline_rounded, size: 20.sp), text: 'Professional'),
            Tab(icon: Icon(Icons.school_outlined, size: 20.sp), text: 'Teaching'),
            Tab(icon: Icon(Icons.folder_outlined, size: 20.sp), text: 'Documents'),
          ],
        ),
      ),
      body: TabBarView(
      ...
      child: Column(
        children: [
          ...
          _card(Column(children: [
            ...
            Text('JPG, PNG or GIF · Max size 5MB', style: GoogleFonts.inter(fontSize: 10.sp, color: AppColors.textLight)),
          ])),
          ...
          _card(Column(children: [
            ...
            Row(children: [
              ...
              SizedBox(width: 12.w),
              Expanded(child: _tf('Date of Birth', _dobCtrl, suffix: Icon(Icons.calendar_month_outlined, size: 18.sp, color: AppColors.textLight))),
            ]),
            SizedBox(height: 14.h),
            Row(children: [
              Expanded(child: _tf('Mobile Number', _mobileCtrl, keyboard: TextInputType.phone)),
              SizedBox(width: 12.w),
              Expanded(child: _tf('Email Address', _emailCtrl, keyboard: TextInputType.emailAddress)),
            ]),
            SizedBox(height: 14.h),
          ...
          _card(Column(children: [
            ...
            SizedBox(height: 14.h),
            _tf('Phone Number', _emergPhCtrl, keyboard: TextInputType.phone),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          SizedBox(height: 16.h),
          _card(Column(children: [
            _tf('LinkedIn Profile', _linkedinCtrl, prefix: Icon(Icons.link_rounded, size: 18.sp, color: AppColors.textLight)),
            SizedBox(height: 14.h),
            _tf('Website of Profile', _websiteCtrl, prefix: Icon(Icons.language_rounded, size: 18.sp, color: AppColors.textLight)),
        ...
        children: [
          _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Wrap(
              ...
              children: [
                ..._subjects.map((s) => _chip(s, onRemove: () => setState(() => _subjects.remove(s)))),
                GestureDetector(
                  onTap: _addSubject,
                  child: Container(
                    ...
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      SizedBox(width: 4.w),
                      Text('Add', style: GoogleFonts.inter(fontSize: 12.sp, fontWeight: FontWeight.w700, color: widget.theme.primary)),
                    ]),
                  ),
                ),
              ],
          ...
          _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            SizedBox(height: 10.h),
            Row(children: ['Online', 'Offline', 'Both'].map((m) => Padding(
              ...
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: _prefMode == m ? widget.theme.primary : Colors.white,
                    borderRadius: BorderRadius.circular(10.r),
                    border: Border.all(color: _prefMode == m ? widget.theme.primary : AppColors.border),
                  ),
                  child: Text(m, style: GoogleFonts.inter(fontSize: 12.sp, fontWeight: FontWeight.w700, color: _prefMode == m ? Colors.white : AppColors.textMedium)),
                ),
              ),
            )).toList()),
            SizedBox(height: 14.h),
            ...
            Wrap(
              ...
              children: _allDays.map((d) => GestureDetector(
                onTap: () => setState(() => _availability.contains(d) ? _availability.remove(d) : _availability.add(d)),
                child: Container(
                  padding: EdgeInsets.symmetric(horizontal: 10.w, vertical: 8.h),
                  decoration: BoxDecoration(
                    color: _availability.contains(d) ? widget.theme.primary : Colors.white,
                    borderRadius: BorderRadius.circular(8.r),
                    border: Border.all(color: _availability.contains(d) ? widget.theme.primary : AppColors.border),
                  ),
                  child: Text(d, style: GoogleFonts.inter(fontSize: 11.sp, fontWeight: FontWeight.w700, color: _availability.contains(d) ? Colors.white : AppColors.textLight)),
                ),
              )).toList(),
            ),
            SizedBox(height: 14.h),
            Row(children: [
              Expanded(child: _tf('Preferred Time Slots (Start)', _startTimeCtrl, suffix: Icon(Icons.access_time_rounded, size: 18.sp, color: AppColors.textLight))),
              SizedBox(width: 12.w),
              Expanded(child: _tf('End Time', _endTimeCtrl, suffix: Icon(Icons.access_time_rounded, size: 18.sp, color: AppColors.textLight))),
            ]),
            SizedBox(height: 14.h),
            _tf('Average Students per Class', _avgStudCtrl, keyboard: TextInputType.number),
            SizedBox(height: 14.h),
      ...
      children: [
        Padding(
          ...
          child: Row(
            ...
            children: [
              Text('Add and manage your documents', style: GoogleFonts.inter(fontSize: 12.sp, color: AppColors.textLight)),
              GestureDetector(
                onTap: () async {
                ...
                child: Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    SizedBox(width: 4.w),
                    Text('Add Document', style: GoogleFonts.inter(fontSize: 12.sp, fontWeight: FontWeight.w700, color: Colors.white)),
                  ]),
                ),
              ),
            ],
              ...
              return Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(doc['name']!, style: GoogleFonts.inter(fontSize: 13.sp, fontWeight: FontWeight.w700, color: AppColors.textDark), maxLines: 1, overflow: TextOverflow.ellipsis),
                    SizedBox(height: 3.h),
                    Text('${doc['type']} · ${doc['size']}', style: GoogleFonts.inter(fontSize: 11.sp, color: AppColors.textLight)),
                  ])),
                  SizedBox(width: 8.w),
                  Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                    Text(doc['date']!, style: GoogleFonts.inter(fontSize: 10.sp, color: AppColors.textLight)),
                    SizedBox(height: 4.h),
                    GestureDetector(
                      onTap: () => _showDocMenu(i),
                      child: Icon(Icons.more_vert_rounded, color: AppColors.textLight, size: 20.sp),
                    ),
                  ]),
                ]),
              );
  ...
  void _showDocMenu(int index) {
    showModalBottomSheet(context: context, shape: RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(24.r))), builder: (_) => Padding(
  ...
  Widget _card(Widget child, {String? label}) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        if (label != null) ...[
          Text(label, style: GoogleFonts.inter(fontSize: 13.sp, fontWeight: FontWeight.w800, color: AppColors.textDark)),
          SizedBox(height: 16.h),
        ],
        child,
      ]),
  ...
  Widget _tf(String label, TextEditingController ctrl, {int maxLines = 1, TextInputType? keyboard, Widget? suffix, Widget? prefix}) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(label, style: GoogleFonts.inter(fontSize: 11.sp, fontWeight: FontWeight.w600, color: AppColors.textMedium)),
      SizedBox(height: 6.h),
      TextFormField(
        controller: ctrl,
        maxLines: maxLines,
  ...
  InputDecoration _inputDec({Widget? suffix, Widget? prefix}) => InputDecoration(
    suffixIcon: suffix,
    prefixIcon: prefix,
    filled: true,
  ...
  Widget _chip(String label, {VoidCallback? onRemove}) {
    return Container(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        if (onRemove != null) ...[
          SizedBox(width: 6.w),
          GestureDetector(onTap: onRemove, child: Icon(Icons.close_rounded, size: 14.sp, color: widget.theme.primary)),
        ],
      ]),
    );
  }
  ...
  Widget _socialField(IconData icon, Color color, TextEditingController ctrl) {
    return Row(children: [
      ...
      Expanded(child: TextFormField(
        controller: ctrl,
        style: GoogleFonts.inter(fontSize: 12.sp, fontWeight: FontWeight.w600, color: AppColors.textDark),
        decoration: _inputDec(),
      )),
    ]);
  }
