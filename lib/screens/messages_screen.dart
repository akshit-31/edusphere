...
class _MessagesScreenState extends State<MessagesScreen> {
  ...
  void didUpdateWidget(covariant MessagesScreen oldWidget) {
    ...
    if (widget.isActive && !oldWidget.isActive) {
      dev.log(
          '🔄 MessagesScreen tab became active, checking messages seen status...',
          name: 'MessagesScreen');
  ...
  Future<void> _initializeRealTimeUsers() async {
    try {
      ...
      dev.log('👤 MessagesScreen user: $_currentUserName ($_currentUserRole)',
          name: 'MessagesScreen');
  ...
  void _processStreamedMessages(List<Map<String, dynamic>> data) {
    ...
    for (final row in sorted) {
      ...
      final bool effectivelySeen = isSeen ||
          (!isMe &&
              widget.isActive &&
              _active != null &&
              _active!.id == contactId);
      final msg = _Msg(
        isMe ? 'me' : 'them',
        text,
        formattedTime,
    ...
    setState(() {
      for (final contactId in chatMessages.keys) {
        ...
        final latestDateTime = latestDateTimeMap[contactId] ??
            DateTime.fromMillisecondsSinceEpoch(0);

        final contactInfo = _availableContacts.firstWhere(
          (c) => c['id'] == contactId,
          orElse: () =>
              {'id': contactId, 'name': 'EduSphere Contact', 'role': 'Chat'},
        );
  ...
  void _sendRichMessage(String text) async {
    ...
    setState(() {
      final tempMsg = _Msg('me', text, 'Now',
          isSeen: false, id: 'temp_${DateTime.now().millisecondsSinceEpoch}');
      ...
      try {
        ...
        if (result != null && result.files.isNotEmpty) {
          ...
          try {
            ...
            dev.log('⚠️ Image path is unavailable on this platform: $pathError',
                name: 'MessagesScreen');
          }
          _sendRichMessage('[Image] $imgPath');
        }
      } catch (ex) {
        dev.log('⚠️ Image fallback selection error: $ex',
            name: 'MessagesScreen');
        if (mounted) {
          showToast(
              context, 'Unable to capture or select image on this device');
        }
      }
    }
  }
    ...
    try {
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        try {
          ...
          dev.log('⚠️ File path is unavailable on this platform: $pathError',
              name: 'MessagesScreen');
        }
  ...
  Widget _listView(BuildContext context) {
    ...
    final filteredChats = _chats.where((c) {
      if (_searchQuery.isNotEmpty &&
          ...
          !c.preview.toLowerCase().contains(_searchQuery.toLowerCase())) {
        return false;
      }
      return true;
    }).toList();
            ...
            child: filteredChats.isEmpty
                    ...
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.chat_bubble_outline_rounded,
                            size: 60.sp, color: AppColors.textLight),
                        SizedBox(height: 12.h),
                        Text(
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      final latestMsg = c.messages.isNotEmpty
                          ? c.messages.last.text
                          : c.preview;
                      final latestTime =
                          c.messages.isNotEmpty ? c.messages.last.time : c.time;
                      final isOutgoing =
                          c.messages.isNotEmpty && c.messages.last.from == 'me';
  ...
  Widget _buildCommunityFeedContent() {
    final filteredPosts = _communityPosts.where((post) {
      ...
      return post.category.trim().toLowerCase() ==
          _communityFilter.trim().toLowerCase();
    }).toList();
        ...
        children: [
          ...
          _isLoadingCommunity
              ? SizedBox(
                  ...
                  child: const Center(
                      child:
                          CircularProgressIndicator(color: Color(0xFF1A6FDB))),
                )
  ...
  Widget _buildCommunityStatsRow() {
    ...
    int postedToday = _communityPosts.where((post) {
      ...
      if (time.contains('day') &&
          !time.contains('today') &&
          !time.contains('hours ago') &&
          !time.contains('mins ago')) {
        return false;
      }
      return true;
    }).length;
          ...
          return GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color:
                      isSel ? const Color(0xFF1A6FDB) : const Color(0xFFE2E8F0),
                  width: 1.w,
                ),
              ),
              child: Row(
        ...
        children: [
          ...
          Padding(
            ...
            child: Text(
              ...
              textAlign: TextAlign.center,
              style: AppTypography.caption
                  .copyWith(color: const Color(0xFF6B7A90), height: 1.4),
            ),
          ),
  ...
  Widget _buildPollWidget(CommunityPostModel post) {
    int totalVotes = post.pollOptions
        .fold(0, (sum, opt) => sum + (opt['votes'] as int? ?? 0));
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            post.content,
            style: AppTypography.caption
                .copyWith(color: const Color(0xFF495057), height: 1.5),
          ),
                  ...
                  SafeArea(
                    child: Container(
                      ...
                      decoration: const BoxDecoration(
                        color: Colors.white,
                        border:
                            Border(top: BorderSide(color: Color(0xFFE9F0F8))),
                      ),
                      child: Row(
                        children: [
                          Expanded(
  ...
  Widget _chatView(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(
          0xFFECE5DD), // Classic WhatsApp beige wallpaper background
      appBar: AppBar(
                  ...
                  bubbleContent = Row(
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color:
                              isMe ? const Color(0xFFC7EFA6) : Colors.grey[200],
                          shape: BoxShape.circle,
                        ),
                        child: const Center(
                      ...
                      children: [
                        if (isFile)
                          InkWell(
                            onTap: () {
                              final filePath =
                                  m.text.substring('[File] '.length);
                              showToast(context,
                                  'Opening file: ${filePath.split('/').last.split('\\').last}');
                            },
                            child: bubbleContent,
                          )
                        else if (isImage)
                            ...
                            if (isMe) ...[
                              ...
                              Icon(
                                ...
                                color: m.isSeen
                                    ...
                                    : Colors.black
                                        .withValues(alpha: 0.35), // Grey ticks
                              ),
                            ],
                          ],
                        ),
  ...
  void _startCall(bool isVideo) {
    ...
    _sendCallSignal('incoming_call', {
      ...
      'caller_name': _currentUserName.isNotEmpty
          ? _currentUserName
          : (_currentUserRole == 'teacher' ? 'Teacher' : 'Student'),
      ...
      if (mounted && _callState == _CallState.active) {
        setState(() {
          _waveHeights = List.generate(
              ...
              (index) => (10 +
                      (index * 5) +
                      (double.tryParse((index * 6).toString()) ?? 10.0))
                  .h);
        ...
        child: Column(
          children: [
            ...
            Text(
              ...
              style: AppTypography.caption
                  .copyWith(color: Colors.white60, letterSpacing: 1.5),
            ),
            SizedBox(height: 20.h),
            Text(
              _activeCallUser,
            ...
            Text(
              ...
              style: AppTypography.small.copyWith(
                  color: _callState == _CallState.incoming
  ...
  Widget _buildAvatarCircle(Color themeColor) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border:
            Border.all(color: themeColor.withValues(alpha: 0.3), width: 4.w),
        boxShadow: [
          BoxShadow(
      ...
      children: [
        ...
        IconButton(
          ...
          onPressed: () {
            ...
            showToast(
                context, _isMuted ? 'Microphone Muted' : 'Microphone Active');
          },
        ),
        // Red End Call Button
        GestureDetector(
...
class _Chat {
  ...
  _Chat(this.id, this.name, this.role, this.time, this.unread, this.online,
      this.preview, this.messages,
