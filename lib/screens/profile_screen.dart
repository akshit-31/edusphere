  ...
  Future<void> _loadStudentDataFromSupabase() async {
    try {
      ...
      if (widget.studentId != null) {
        studentRes = await client
            .from('Student')
            .select('id, currentClassId, sectionId, admissionNumber, rollNumber, joiningDate, medium, religion, caste, nationality, emergencyPhone, User(firstName, lastName, email, phone, dateOfBirth, gender, bloodGroup, address), Class(name), Section(name)')
      ...
      setState(() {
        ...
        setState(() {
          ...
          _admissionNo = data['admissionNumber'] as String? ?? 'ADM-2026-024';
          _studentClass = classMap['name'] as String? ?? 'Grade 12';
          _section = sectionMap['name'] as String? ?? 'A';
          _rollNo = data['rollNumber']?.toString() ?? '24';
          
          _batch = '2024-25';
          _medium = data['medium'] as String? ?? 'ENGLISH';
          
          final joinDateStr = data['joiningDate'] as String?;
          if (joinDateStr != null) {
            try {
          ...
          _emergencyInfo = data['emergencyPhone'] as String? ?? 'UNSET';
      ...
      try {
        ...
        if (parentsList.isNotEmpty) {
          ...
          _studentBloodGroup = userMap['bloodGroup'] as String? ?? '—';
          _religion = data['religion'] as String? ?? 'HINDU';
          _casteGroup = data['caste'] as String? ?? 'GENERAL';
          _nationality = data['nationality'] as String? ?? 'INDIAN';
        });

        final studentId = data['id'] as String;
        final String? sectionId = data['sectionId'] as String?;
