...
async function testRegions() {
  console.log('--- Testing Pooled Connections (Port 6543) for bstevdkjqjzaglayicdg with require & rejectUnauthorized ---');
  for (const region of regions) {
    const host = `aws-0-${region}.pooler.supabase.com`;
    const user = `postgres.${projectRef}`;
    const uri = `postgresql://${user}:${password}@${host}:6543/postgres?sslmode=require`;
