...
function query() {
  const url = `https://bstevdkjqjzaglayicdg.supabase.co/rest/v1/ExamMark?select=*,ExamResult!inner(id,examId)&ExamResult.examId=eq.${examId}`;
  const options = {
    headers: {
      "apikey": key,
  ...
  https.get(url, options, (res) => {
    ...
    res.on('end', () => {
      ...
      try {
        ...
        console.log("ExamMarks data count:", parsed.length);
        console.log("First mark:", JSON.stringify(parsed[0], null, 2));
      } catch(e) {
        console.log(data);
      }
    });
